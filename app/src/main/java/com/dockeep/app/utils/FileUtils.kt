    ...
    companion object {
        ...
        fun createHierarchicalZipFile(context: Context, userName: String, mainUserDocuments: Map<String, List<String>>, personDocuments: Map<String, Map<String, List<String>>>): File? {
            return try {
                ...
                val timeStamp = SimpleDateFormat("yyyyMMdd_HHmm", Locale.getDefault()).format(Date())
                val safeUserName = sanitizeForPath(userName).ifBlank { "Documents" }
                val zipFileName = "${safeUserName}_Documents_$timeStamp.zip"
                val zipFile = File(exportsDir, zipFileName)
                ...
                ZipOutputStream(FileOutputStream(zipFile)).use { zipOutputStream ->
                    ...
                    for ((documentName, imagePaths) in mainUserDocuments) {
                        ...
                        val sanitizedDocumentName = sanitizeForPath(documentName).ifBlank { "Untitled" }
                    
                        // Prefix document name with user's name
                        val prefixedDocumentName = "$safeUserName's $sanitizedDocumentName"
                    ...
                    for ((personName, documents) in personDocuments) {
                        ...
                        val sanitizedPersonName = sanitizeForPath(personName).ifBlank { "Unnamed" }
        ...
        /**
         * Reduces a name to something safe as a path segment.
         *
         * Anything outside letters, digits, dash, underscore and space becomes
         * an underscore, so a name carrying a slash cannot introduce a folder
         * level of its own inside the archive.
         */
